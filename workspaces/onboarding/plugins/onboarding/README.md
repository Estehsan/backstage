# @backstage-community/plugin-onboarding
...
yarn --cwd packages/app add @backstage-community/plugin-onboarding
```

Add the plugin page to your app routes in `packages/app/src/App.tsx`:

```tsx
import { OnboardingPage } from '@backstage-community/plugin-onboarding';
...
```bash
# From your Backstage root directory
yarn --cwd packages/backend add @backstage-community/plugin-onboarding-backend
```

Add the backend plugin to `packages/backend/src/index.ts`:

```ts
backend.add(import('@backstage-community/plugin-onboarding-backend'));
```
