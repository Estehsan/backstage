    ...
    const activeJoiners = allProgress
      ...
      .map(p => {
        ...
        return {
          userId: p.userId,
          displayName:
            (member?.spec as Record<string, unknown>)?.profile &&
            typeof (member?.spec as Record<string, unknown>)?.profile ===
              'object'
              ? ((
                  (member?.spec as Record<string, unknown>)?.profile as Record<
                    string,
                    unknown
                  >
                )?.displayName as string) ?? p.userId
              : p.userId,
          role: p.templateName,
